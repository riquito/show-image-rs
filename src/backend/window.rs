...
impl<'a> WindowHandle<'a> {
	...
	pub fn set_outer_position(&self, position: impl Into<glam::IVec2>) {
		...
		self.window()
			.window
			.set_outer_position(winit::dpi::Position::Physical(winit::dpi::PhysicalPosition::new(
	...
	pub fn inner_size(&self) -> glam::UVec2 {
		let size = self.window().window.surface_size();
	...
	pub fn set_inner_size(&mut self, size: impl Into<glam::UVec2>) {
		...
		let _ = self
			.window_mut()
			.window
			.request_surface_size(winit::dpi::PhysicalSize::new(size.x, size.y).into());
		self.window().window.request_redraw();
	}
	...
	pub fn calculate_uniforms(&self) -> WindowUniforms {
		if let Some(image) = &self.image {
			...
			if !self.preserve_aspect_ratio {
				...
				let window_size = glam::UVec2::new(self.window.surface_size().width, self.window.surface_size().height).as_vec2();
